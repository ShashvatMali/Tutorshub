...
if (!class_exists('Tuturn_Nav_Menu_Widget')) {
    class Tuturn_Nav_Menu_Widget extends WP_Widget {
        ...
        public function __construct() {
            $widget_ops = array(
                'description'                 => __( 'Add a navigation menu to your sidebar.','tuturn' ),
                'customize_selective_refresh' => true,
                'show_instance_in_rest'       => true,
            );
            parent::__construct( 'tuturn_nav_menu', __( 'Tuturn navigation Menu','tuturn' ), $widget_ops );
        }
        ...
        public function form( $instance ) {
            ...
            <p class="nav-menu-widget-no-menus-message" <?php echo do_shortcode( $not_empty_menus_style ); ?>>
                ...
                printf( __( 'No menus have been created yet. <a href="%s">Create some</a>.','tuturn' ), esc_attr( $url ) );
                ?>
            </p>
            <div class="nav-menu-widget-form-controls" <?php echo do_shortcode( $empty_menus_style ); ?>>
                <p>
                    <label for="<?php echo do_shortcode( $this->get_field_id( 'title' )); ?>"><?php _e( 'Title:','tuturn' ); ?></label>
                ...
                <p>
                    <label for="<?php echo do_shortcode( $this->get_field_id( 'nav_menu' )); ?>"><?php _e( 'Select Menu:','tuturn' ); ?></label>
                    <select id="<?php echo do_shortcode( $this->get_field_id( 'nav_menu' )); ?>" name="<?php echo do_shortcode( $this->get_field_name( 'nav_menu' )); ?>">
                        <option value="0"><?php _e( '&mdash; Select &mdash;','tuturn' ); ?></option>
                ...
                <?php if ( $wp_customize instanceof WP_Customize_Manager ) : ?>
                    <p class="edit-selected-nav-menu" style="<?php echo do_shortcode( $nav_menu_style ); ?>">
                        <button type="button" class="button"><?php _e( 'Edit Menu','tuturn' ); ?></button>
                    </p>
                <?php endif; ?>
            </div>
            <?php
...
//register widget
function tuturn_nav_menu_widgets() {
	register_widget( 'Tuturn_Nav_Menu_Widget' );
}
add_action( 'widgets_init', 'tuturn_nav_menu_widgets' );
