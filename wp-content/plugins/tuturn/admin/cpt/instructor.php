...
/**
 * 
 * Class 'Tuturn_Admin_CPT_Service_Provider' defines the custom post type Tuturn instructor
 *
 * @package     Tuturn
 ...
 * @since       1.0
 */
class Tuturn_Admin_CPT_Service_Provider {

	/**
	 * Tuturn instructor post type
	 *
	 * @since    1.0.0
	 * @access   public
	 */
	public function __construct() {
		...
		add_action('views_edit-tuturn-instructor', array(&$this, 'tuturn_add_admin_quick_link'));
		add_filter('manage_tuturn-instructor_posts_columns', array(&$this, 'volunteers_columns_add'));
        add_action('manage_tuturn-instructor_posts_custom_column', array(&$this, 'volunteers_columns'),10, 2);
		add_action( 'add_meta_boxes',	array( $this, 'tuturn_instructor_add_meta_box'));
	...
	*/
	public function init_post_type() {
		...
		add_action( 'wp_ajax_tuturn_instructor_assign_package',	array( $this, 'tuturn_instructor_assign_package'));
	}

	public function tuturn_add_admin_quick_link( $views ) {
		...
		$views['tuturn-download-hours'] = sprintf( '<a href="%s" class="tuturn-download-hours">%s</a>',
			admin_url( "edit.php?post_type={$post_type}&download_instructor_hours=true" ),
	...
	public function download_tutoring_log_csv(){
		...
		if(!empty($_GET['download_instructor_hours']) ){
			$file_name	= 'tutoring_log';
			$file_name	= "instructors-hours-" . date('Ymd') . ".xls";
			...
			foreach($users as $user){
				$hours_array    = tuturn_hours_data_by_meta(array(array('key' => 'instructor_id', 'value' => $user->ID )));
				$total_hours	= !empty($hours_array['total']) ? $hours_array['total'] : 0;
				$approved_hours	= !empty($hours_array['completed']) ? $hours_array['completed'] : 0;
				$declined_hours	= !empty($hours_array['pending']) ? $hours_array['pending'] : 0;
	...
	/**
	 *Regirster Tuturn instructor post type
	*/ 
	public function register_posttype() {
		$labels = array(
			'name'                  => esc_html__( 'Instructors', 'tuturn' ),
			'singular_name'         => esc_html__( 'Instructor', 'tuturn' ),
			'menu_name'             => esc_html__( 'Instructor', 'tuturn' ),
			'name_admin_bar'        => esc_html__( 'Instructor', 'tuturn' ),
			'parent_item_colon'     => esc_html__( 'Parent instructor', 'tuturn' ),
			'all_items'             => esc_html__( 'All instructors', 'tuturn' ),
			'add_new_item'          => esc_html__( 'Add new instructor', 'tuturn' ),
			'add_new'               => esc_html__( 'Add new instructor', 'tuturn' ),
			'new_item'              => esc_html__( 'New instructor', 'tuturn' ),
			'edit_item'             => esc_html__( 'Edit instructor', 'tuturn' ),
			'update_item'           => esc_html__( 'Update instructor', 'tuturn' ),
			'view_item'             => esc_html__( 'View instructors', 'tuturn' ),
			'view_items'            => esc_html__( 'View instructors', 'tuturn' ),
			'search_items'          => esc_html__( 'Search instructors', 'tuturn' ),
		);
		
		$args = array(
			'label'                 => esc_html__( 'Instructors', 'tuturn' ),
			'description'           => esc_html__( 'All instructors.', 'tuturn' ),
			...
			'rest_base'             => 'tuturn-instructor',
			'rest_controller_class' => 'WP_REST_Posts_Controller',
		);
		
		register_post_type( apply_filters('instructor_post_type_name', 'tuturn-instructor'), $args );
	}	

	public function tuturn_instructor_assign_package(){
		$json		= array();
     ...
     */
    public function tuturn_instructor_add_meta_box( $post_type ) {
		add_meta_box(
		...
		add_meta_box(
			'tuturn_update_featured_price',
			esc_html__( 'Hourly price', 'tuturn' ),
			array( $this, 'tuturn_update_featured_price_fields' ),
			'tuturn-instructor',
			'side',
			'high'
		);
     ...
     */
	public function tuturn_update_featured_price_fields( $post ) {
		$hourly_rate 	= get_post_meta($post->ID, 'hourly_rate',true);
		$hourly_rate	= isset($hourly_rate) ? $hourly_rate : '';
		?>
		<label class="tu-label"><strong><?php esc_html_e('Hourly rate', 'tuturn'); ?></strong></label>
		<input type="text" name="hourly_rate" value="<?php echo esc_attr($hourly_rate);?>" class="form-control">
	<?php
	}
		...
		if(class_exists('WooCommerce')){
			...
			if(isset($tuturn_packages) && is_array($tuturn_packages) && count($tuturn_packages)>0){?>
				<div class="tu-package-plan">
					...
					<button type="button" id="tu-assign-package" data-profile_id="<?php echo intval($post->ID);?>" data-user_id="<?php echo intval($user_id);?>" class="button button-primary button-large"><?php esc_html_e('Assign package', 'tuturn');?></button>
				</div>
				<?php
			}
		}
...
new Tuturn_Admin_CPT_Service_Provider();
