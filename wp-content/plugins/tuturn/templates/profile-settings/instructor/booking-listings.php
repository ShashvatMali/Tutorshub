...
<div class="tu-bookings">
    <div class="tu-dbwrapper">
        <?php tuturn_get_template_part('profile-settings/booking', 'search-form', $args);?>
        <?php if (!empty($customer_orders->orders)) {
            ...
            foreach ($customer_orders->orders as $order) {
                ...
                $_booking_slots	= get_post_meta( $order->get_id(), '_booking_slots',true );
                ...
                <div class="tu-bookingwrapper">
                    <div class="tu-bookingperson">
                        <?php if(!empty($avatar)){?>
                            <figure>
                                <img src="<?php echo esc_url($avatar);?>" alt="<?php echo esc_html($username);?>">
                            </figure>
                        <?php }?>
                        <div class="tu-bookername">
                            <h4><a href="<?php echo esc_url(get_permalink($profile_id));?>" target="_blank"><?php echo esc_html($order->get_formatted_billing_full_name()); ?></a><span class="tu-tagstatus<?php echo esc_attr($class);?>"><?php echo esc_html($status_label);?></span></h4>
                    ...
                    <ul class="tu-bookingonfo">
                        ...
                        if(!empty($booked_data['booked_slots'])){
                            ...
                            if(!empty($start_appointment_date)){?>
                                <li>
                                    <span><?php esc_html_e('Appointment date', 'tuturn');?>:</span>
                                    <h6><?php  echo date_i18n('D j F, Y', strtotime($start_appointment_date));?></h6>
                                    <a href="javascript:void(0);" class="tu-bookingdetails" data-user_id="<?php echo intval($student_id);?>" data-booking_id="<?php echo (int)$order->get_id();?>"><?php esc_html_e('See booking details', 'tuturn'); ?></a>
                                </li>                        
                                <?php 
                            }
                        }?>
                        <?php if(!empty($user_information)){?>
                            <li>
                        ...
                        <?php } else {
                            ...
                            $username           = tuturn_get_username($profile_id);
                            $user_data          = get_post_meta($profile_id, 'profile_details', true);
                        ...
                        <?php if(!empty($user_information['info_desc'])){?>
                            <li>
                                ...
                                <p><?php echo esc_html($user_information['info_desc']);?></li></p>
                            </li>
                        <?php }
                        if(!empty($meeting_detail)){?>
                            <li>
                                <span><?php esc_html_e('Meeting detail:', 'tuturn');?></span>
                                <div class="meeting-detail tu-meeting-detail">
                                    ...
                                    if(!empty($meeting_detail['meeting_date'])){?>
                                        <span><i class="icon icon-calendar"></i><?php esc_html_e('Last updated:','tuturn')?> <?php echo esc_html($meeting_detail['meeting_date']);?></span>
                                    <?php } ?>
                                </div>                               
                            </li>
                        <?php } ?>
                    ...
                    if(!empty($booking_status) && $booking_status == 'pending' && (strtotime($booking_start_time) > strtotime($gmt_time))){?>
                        <div class="tu-btnlist">
                            <a class="tu-sb-sliver tu-booking-modal" data-order_id="<?php echo (int)$order->get_id();?>" data-action_type="decline" href="javascript:void(0);"><?php esc_html_e('Decline', 'tuturn');?></a>
                            <a class="tu-pb tu-btn tu-booking-modal" data-order_id="<?php echo (int)$order->get_id();?>" data-action_type="approve" href="javascript:void(0);"><?php esc_html_e('Approve appointment', 'tuturn');?></a>
                            <?php do_action('tuturn_guppy_message', $args);?>
                        </div>
                    <?php } elseif(!empty($booking_status) && $booking_status == 'publish' && (strtotime($booking_start_time) > strtotime($gmt_time))){?>
                        <div class="tu-btnlist">
                            <a class="tu-sb-sliver tu-booking-modal" data-order_id="<?php echo (int)$order->get_id();?>" data-action_type="decline" href="javascript:void(0);"><?php esc_html_e('Decline', 'tuturn');?></a>
                            <div class="tu-profilewrapper">
                                <a class="tu-secbtn tu-meeting-modal" data-meeting='<?php echo json_encode($meeting_detail); ?>' data-order_id="<?php echo (int)$order->get_id();?>" data-action_type="meeting-inst" href="javascript:void(0);"><?php esc_html_e('Add/edit meeting details', 'tuturn');?><i class="icon icon-plus"></i></a>
                            </div>
                            <?php do_action('tuturn_guppy_message', $args);?>
                        </div>
                    <?php } elseif(!empty($booking_status) && ($booking_status == 'declined' || $booking_status == 'cancelled')){                       
                        ...
                        if(!empty($decline_reason) || !empty($decline_description)){?>  <div class="tu-decnoti">                         
            ...
            <div class="tu-bookingwrapper tu-booking-epmty-field">
...
<?php tuturn_get_template_part('profile-settings/'.$userType.'/booking', 'js-templates');?>
<?php
$script = "
