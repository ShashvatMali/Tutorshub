...
class TheGem_Team extends Widget_Base {
	...
	protected function register_controls() {
		...
		if( !empty(thegem_get_option('team_persons_page_enable')) ) {

			$this->add_control(
				'link_to_person_page', [
					'label' => __( 'Link to Person Page', 'thegem' ),
					'default' => 'no',
					'type' => Controls_Manager::SWITCHER,
					'label_on' => __( 'On', 'thegem' ),
					'label_off' => __( 'Off', 'thegem' ),
	...
	protected function container_styles( $control ) {
		...
		$control->start_controls_tab( 'container_tab_hover', [
...
/*		$control->start_controls_section(
	...
	public function render() {
		...
		if ( $persons->have_posts() ) {
			...
			<div class="row gem-team gem-team-style-<?php echo esc_attr($preset); ?> <?php if (!empty($settings['equal_height'])): ?>gem-team-equal-height<?php endif;?>" <?php echo $data_attr; ?>>
				...
				<?php
					while ( $persons->have_posts() ) {
						...
						if(!empty($settings['skin_source']) && $settings['skin_source'] === 'builder' && !empty($settings['loop_builder'])) {
							$post_id = get_the_ID();
							echo '<div class="' . esc_attr($columns_classes) . ' thegem-wrap inline-column"><div id="' . esc_attr($post_id) . '" class="' . esc_attr(implode(' ', get_post_class(array('team-person', 'loop-builder'), $post_id))) . '"><div class="thegem-template-wrapper thegem-template-loop-item thegem-template-' . esc_attr($settings['loop_builder']) . ' thegem-loop-post-' . esc_attr($post_id) . '">';
							...
							if(empty($thegem_link) && !empty($settings['link_to_person_page']) && !empty(thegem_get_option('team_persons_page_enable'))) {
								$thegem_link = get_the_permalink();
							}
							...
							foreach ( $thegem_item_data['additional_social_links'] as $thegem_social ) {
								thegem_additionals_socials_enqueue_style( $thegem_social['social'] );
