...
class TheGem_Pricing_Table extends Widget_Base
{
	...
	protected function features_styles($control) {
		...
		$control->add_control(
			...
			[
				...
				'label_block' => false,
				'selectors' => [
					'{{WRAPPER}} figure.pricing-row.strike' => 'color: {{VALUE}};',
				],
			]
		);
	...
	public function render()
	{
		...
		if (isset($settings['content_header_title'])) {
			$settings['content_header_title'] = do_shortcode($settings['content_header_title']);
		}

		if (isset($settings['content_pricing_price'])) {
			$settings['content_pricing_price'] = do_shortcode($settings['content_pricing_price']);
		}

		if (isset($settings['content_pricing_period'])) {
			$settings['content_pricing_period'] = do_shortcode($settings['content_pricing_period']);
		}

		if (isset($settings['content_footer_button_text'])) {
			$settings['content_footer_button_text'] = do_shortcode($settings['content_footer_button_text']);
		}

		if (isset($settings['content_label_text'])) {
			$settings['content_label_text'] = do_shortcode($settings['content_label_text']);
		}

		if (isset($settings['features_list']) && !empty($settings['features_list'])) {
			foreach ($settings['features_list'] as $key => $item) {
				$settings['features_list'][$key]['feature_text'] = do_shortcode($item['feature_text']);
			}
		}
