...
class TheGem_QuickFinders extends Widget_Base {
	...
	public function render() {
		...
		if (isset($settings['qf_list']) && !empty($settings['qf_list'])) {
			foreach ($settings['qf_list'] as $key => $item) {
				$settings['qf_list'][$key]['qf_item_title'] = do_shortcode($item['qf_item_title']);
			}
		}
		...
		if ( in_array( $settings['thegem_elementor_preset'], array_keys( $this->filter_skins_by( $skins, 'group', 'grid' ) ) ) ) {
			...
			if ( 'yes' === ( $settings['equal_height'] ) )  {
				$this->add_render_attribute( 'main_wrap', 'class', 'quickfinder-equal-height' );
			}
