...
<li <?php post_class($thegem_classes); ?> style="padding: calc(<?= $settings['container_margin']['size'].$settings['container_margin']['unit'] ?>/2); <?php
if (!$is_size_item && $settings['layout'] == 'justified' && $settings['ignore_highlights'] == 'yes' && !empty($active_filter) && empty(array_intersect($active_filter, $thegem_classes))) { echo 'display: none;'; } ?>">
	<?php if (!$is_size_item) { ?>
						...
						<div class="overlay <?php if ($settings['thegem_elementor_preset'] == '11'): ?>img-circle<?php endif; ?>">
							...
							<?php if ($thegem_single_icon): ?>
								<a href="<?php echo esc_url($thegem_full_image_url[0]); ?>"
								   ...
								   data-fancybox="gallery-<?php echo esc_attr($gallery_uid); ?>" data-elementor-open-lightbox="no">
							...
							<div class="overlay-content">
								<div class="overlay-content-center">
									<div class="overlay-content-inner">
										...
										<?php if ($settings['title_show'] == 'yes' && !empty($thegem_item->post_excerpt)) : ?>
											<div class="title">
												<?php echo $thegem_item->post_excerpt; ?>
											</div>
										<?php endif; ?>
										<?php if ($settings['description_show'] == 'yes' && !empty($thegem_item->post_content)) : ?>
											<div class="subtitle">
												<?php echo $thegem_item->post_content; ?>
											</div>
										<?php endif; ?>
