...
class TheGem_PortfolioList extends Widget_Base {
	...
	protected function register_controls() {
		...
		foreach ($portfolio_fields as $ekey => $elem) {
			...
			if ($this->is_portfolio_post && ($ekey == 'filter' || $ekey == 'sorting')) {
				$default_show = '';
			} else {
				$default_show = 'yes';
			}
	...
	protected function filter_buttons_style($control) {
		...
		$control->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .portfolio.portfolio-list .portfolio-top-panel' => 'margin-bottom: {{SIZE}}{{UNIT}}',
				]
			]
		);
	...
	public function render() {
		...
		$terms = $settings['content_portfolios_thegem_portfolios'];
		...
		$portfolio_loop = thegem_get_portfolio_list_posts($portfolios_posts, $taxonomy_filter, $page, $items_per_page, $settings['orderby'], $settings['order']);
		...
		if ($portfolio_loop->have_posts()) : ?>
			<?php if (empty($terms) || in_array('0', $terms)) {
			...
			<div class="portfolio-preloader-wrapper">
				...
				$this->add_render_attribute(
					...
					[
						'class' => [
							'portfolio portfolio-grid extended-portfolio-grid portfolio-list no-padding disable-isotope',
		...
		if (is_admin() && Plugin::$instance->editor->is_edit_mode()): ?>
			...
			<script type="text/javascript">
				(function ($) {
					$('.elementor-element-<?php echo $this->get_id(); ?> .portfolio-list').initExtendedPortfolioGrids();
