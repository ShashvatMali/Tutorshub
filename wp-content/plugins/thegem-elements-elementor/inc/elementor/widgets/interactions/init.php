...
//		add_action('elementor/frontend/section/before_render', array($this, 'before_render'));
		...
		add_filter('elementor/element/is_dynamic_content', array($this, 'disable_cache'), 10, 3);
	...
	public function before_render($obj) {
		...
		if(!empty($this->interactions[$data['id']])) {
			$obj->add_render_attribute('_wrapper', 'data-thegem-interactions', wp_json_encode($this->interactions[$data['id']]));
		}
	}

	public function enqueue_scripts() {

		if (!empty($this->interactions) || !empty($GLOBALS['thegem_interactions_activate']) || \Elementor\Plugin::$instance->preview->is_preview_mode()) {
	...
	public function disable_cache($status, $raw_data, $obj) {
		$data = $obj->get_data();
