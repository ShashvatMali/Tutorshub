...
class TheGem_ProductsGrid extends Widget_Base {
	...
	public function get_title() {
		return __('Product Classic Grid (Legacy)', 'thegem');
	}
		...
		$control->add_responsive_control(
			...
			[
				...
				'selectors' => [
					...
					'{{WRAPPER}} .portfolio.products-grid:not(.item-separator) .portfolio-row' => 'margin: calc(-{{SIZE}}{{UNIT}}/2);',
					'{{WRAPPER}} .portfolio.products-grid.item-separator .portfolio-row' => 'margin: 0 calc(-{{SIZE}}{{UNIT}}/2);',
					'{{WRAPPER}} .portfolio.products-grid.fullwidth-columns:not(.item-separator) .portfolio-row' => 'margin: calc(-{{SIZE}}{{UNIT}}/2) 0;',
					'{{WRAPPER}} .portfolio.products-grid.fullwidth-columns.item-separator .portfolio-row' => 'margin: 0;',
	...
	protected function filter_buttons_style($control) {
		...
		$control->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .portfolio.products-grid .portfolio-top-panel' => 'margin-bottom: {{SIZE}}{{UNIT}}',
				]
			]
		);
...
//					'{{WRAPPER}} .portfolio.products-grid .product .new-label' => 'width: {{SIZE}}{{UNIT}}; height: {{SIZE}}{{UNIT}}; margin-top: calc(-{{SIZE}}{{UNIT}}/2); margin-left: calc(-{{SIZE}}{{UNIT}}/2);margin-right: calc(-{{SIZE}}{{UNIT}}/2);',
	...
	public function render() {
		...
		$product_loop = thegem_get_product_posts($terms, $page, $items_on_load, $orderby, $order, $featured_only, $sale_only);
		...
		if ($product_loop && $product_loop->have_posts()) :
			...
			if ($settings['product_separator'] == 'yes' && intval($settings['product_separator_width']['size']) % 2 !== 0) {
				$floor = floor(intval($settings['product_separator_width']['size']) / 2);
				$ceil = ceil(intval($settings['product_separator_width']['size']) / 2); ?>
				<style>
					.elementor-element-<?php echo $portfolio_uid; ?> .portfolio-set .portfolio-item:before {
						transform: translateX(-<?php echo $floor; ?>px) !important;
						top: -<?php echo $floor; ?>px !important;
					}

					.elementor-element-<?php echo $portfolio_uid; ?> .portfolio-set .portfolio-item:after {
						transform: translateX(<?php echo $ceil; ?>px) !important;
						top: -<?php echo $floor; ?>px !important;
					}

					.elementor-element-<?php echo $portfolio_uid; ?> .portfolio-set .portfolio-item .item-separator-box:before {
						transform: translateY(-<?php echo $floor; ?>px) !important;
						left: -<?php echo $floor; ?>px !important;
					}

					.elementor-element-<?php echo $portfolio_uid; ?> .portfolio-set .portfolio-item .item-separator-box:after {
			...
			<div class="portfolio-preloader-wrapper">
				...
				$this->add_render_attribute(
					...
					[
						'class' => [
							'portfolio portfolio-grid extended-portfolio-grid',
							'products-grid',
							'woocommerce',
							'products',
							'no-padding',
				...
				<div <?php echo $this->get_render_attribute_string('products-wrap'); ?>>
					<?php if (($settings['product_show_filter'] == 'yes' && count($terms) > 0) || $settings['product_show_sorting'] == 'yes'): ?>
						<div class="portfolio-top-panel<?php if ($settings['columns'] == '100%'): ?> fullwidth-block<?php endif; ?>">
					...
					<div class="portfolio-row-outer <?php if ($settings['columns'] == '100%'): ?>fullwidth-block no-paddings<?php endif; ?>">
						...
						if ('yes' === ($settings['show_pagination'])) : ?>
							...
							<?php endif; ?>
							<?php
							if ($settings['pagination_type'] == 'more' && $next_page_pagination > 0):
							...
							<?php endif; ?>
							<?php if ($settings['pagination_type'] == 'scroll' && $next_page_pagination > 0): ?>
		...
		if (is_admin() && Plugin::$instance->editor->is_edit_mode()): ?>
			<script>
				...
				widget_settings['<?php echo $grid_uid ?>'] = <?php echo json_encode($settings) ?>;
			</script>
			<script type="text/javascript">
				(function ($) {
					...
					setTimeout(function () {
						...
						$('.elementor-element-<?php echo $this->get_id(); ?> .products-grid').initExtendedPortfolioGrids();
					}, 1000);
