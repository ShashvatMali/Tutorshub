...
function thegem_portfolio_type_options() {
	...
	return $PORTFOLIO_TYPE_OPTIONS;
}


function thegem_portfolio_item_post_type_init() {
	$labels = array(
		'name'               => __('TheGem Portfolio Items', 'thegem'),
	...
	$args = array(
		...
		'query_var'             => !empty(thegem_get_option('portfolio_category_page_enable')),
		'public'                => !empty(thegem_get_option('portfolio_category_page_enable')),
		'rewrite'               => !empty(thegem_get_option('portfolio_category_page_enable')) ? array('slug' => apply_filters('thegem_portfolio_category_rewrite_slug', 'portfolio')) : false,
	);
...
function thegem_options_portfolio_category_rewrite_slug($value) {
	if(sanitize_title(thegem_get_option('portfolio_category_rewrite_slug'))) {
		$value = sanitize_title(thegem_get_option('portfolio_category_rewrite_slug'));
	}
	return $value;
}
add_filter('thegem_portfolio_category_rewrite_slug', 'thegem_options_portfolio_category_rewrite_slug');
...
?>
	<div class="portfolio_item_element_<?php echo $types_index; ?>" id="portfolio_item_type_<?php echo $types_index; ?>_wrapper">
		...
		<?php thegem_print_select_input(thegem_portfolio_type_options(), $item_type['type'], 'thegem_portfolio_item_data[types]['.$types_index.'][type]', 'portfolio_item_type_'.$types_index); ?>
	</div>
...
function thegem_get_sanitize_admin_pf_item_data($post_id = 0, $item_data = array()) {
	...
	foreach ($portfolio_item_data['types'] as $k => $v) {
		...
		$portfolio_type_options = array_keys(thegem_portfolio_type_options());
...
function thegem_select_portfolio_taxonomies() {
	$out = [];
	$taxonomies = get_taxonomies(['object_type' => ['thegem_pf_item']], 'objects');
