...
function thegem_team_person_post_type_init() {
	...
	$args = array(
		'labels'               => $labels,
		'public'               => !empty(thegem_get_option('team_persons_page_enable')),
		'rewrite'              => array('slug' => apply_filters('thegem_team_persons_rewrite_slug', 'team-person'), 'with_front' => false),
		'exclude_from_search'  => empty(thegem_get_option('team_persons_page_enable')),
		'publicly_queryable'   => !empty(thegem_get_option('team_persons_page_enable')),
		'show_ui'              => true,
		'query_var'            => !empty(thegem_get_option('team_persons_page_enable')),
		'hierarchical'         => false,
	...
	$args = array(
		...
		'query_var'             => !empty(thegem_get_option('team_persons_page_enable')),
		'public'                => !empty(thegem_get_option('team_persons_page_enable')),
		'rewrite'               => !empty(thegem_get_option('team_persons_page_enable')) ? array('slug' => 'teams') : false,
	);
...
function thegem_options_team_persons_rewrite_slug($value) {
	if(sanitize_title(thegem_get_option('team_persons_rewrite_slug'))) {
		$value = sanitize_title(thegem_get_option('team_persons_rewrite_slug'));
	}
	return $value;
}
add_filter('thegem_team_persons_rewrite_slug', 'thegem_options_team_persons_rewrite_slug');
...
function thegem_team_persons_register_meta_box($post) {
	...
	add_meta_box('thegem_team_person_custom_fields', __('Person Custom Fields', 'thegem'), 'thegem_team_person_custom_fields_box', 'thegem_team_person', 'normal', 'high');
}
...
function thegem_team_person_custom_fields_box($post) {
	wp_nonce_field('thegem_team_person_custom_fields_box', 'thegem_team_person_custom_fields_box_nonce');
	$team_persons_custom_fields = thegem_getCustomFieldsFromPostTypes('team_persons');
	echo '<p class="meta-options">';
	if(!empty($team_persons_custom_fields) && is_array($team_persons_custom_fields)) {
		foreach($team_persons_custom_fields as $field) {
			$value = get_post_meta($post->ID, $field['key'], true);
			$value = empty($value) ? $field['value'] : $value;
			echo '<label for="thegem_team_persons_custom_field_' . esc_attr($field['key']) . '">' . esc_html($field['title']) . ':</label><br />';
			echo '<input name="thegem_team_persons_custom_field[' . esc_attr($field['key']) . ']" type="text" id="thegem_team_persons_custom_field_' . esc_attr($field['key']) . '" value="' . esc_attr($value) . '" size="60" /><br /><br />';
		}
	} else {
		printf(__('No custom fields found. You can add new custom fields in <a target="_blank" href="%s">Theme Option</a>', 'thegem'), admin_url('admin.php?page=thegem-theme-options#/posttypes/team_persons'));
	}
	echo '</p>';
}
...
function thegem_team_person_additional_socials_list() {
	return array_diff(apply_filters('thegem_socials_icons_list', array(
		...
		'tiktok' => 'TikTok', 'soundcloud' => 'SoundCloud', 'discord' => 'Discord', 'wechat' => 'WeChat',
		'xing' => 'Xing', 'threads' => 'Threads', 'bluesky' => 'Bluesky', 'houzz' => 'Houzz',
...
function thegem_team_person_save_meta_box_data($post_id) {
	...
	if(isset($_POST['thegem_team_person_custom_fields_box_nonce']) && wp_verify_nonce($_POST['thegem_team_person_custom_fields_box_nonce'], 'thegem_team_person_custom_fields_box')) {
		if(isset($_POST['thegem_team_persons_custom_field']) && is_array($_POST['thegem_team_persons_custom_field'])) {
			foreach($_POST['thegem_team_persons_custom_field'] as $key => $val) {
