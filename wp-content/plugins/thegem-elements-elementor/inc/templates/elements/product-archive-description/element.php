...
namespace TheGem_Elementor\Widgets\TemplateProductArchiveDescription;
...
/**
 * Elementor widget for Product Description.
 */

#[\AllowDynamicProperties]
class TheGem_TemplateProductArchiveDescription extends Widget_Base {
	...
	public function get_name() {
		return 'thegem-template-product-archive-description';
	}
	...
	public function get_widget_wrapper() {
		return 'thegem-te-product-archive-description';
	}
	...
	public function get_customize_class($only_parent = false) {
		...
		return ' .'.$this->get_widget_wrapper().' .product-description';
	}
	...
	public function render() {
		...
		$description = !empty($term) && !empty($term->description) ? $term->description : '';
		if(is_post_type_archive('product')) {
			$shop_page_id = wc_get_page_id('shop');
			if($shop_page_id > 0 && get_post($shop_page_id)) {
				$page_data = thegem_get_output_page_settings($shop_page_id);
				$description = $page_data['title_excerpt'];
			} else {
				$description = '';
			}
		}
        ...
        <div <?php if (!empty($params['element_id'])): ?>id="<?=esc_attr($params['element_id']); ?>"<?php endif;?>
            ...
            <<?= $params['text_tag'] ?> class="product-description <?= $text_styled_class ?>">
                <?= $description ?>
            </<?= $params['text_tag'] ?>>
        </div>
...
if(defined('WC_PLUGIN_FILE')) {
	\Elementor\Plugin::instance()->widgets_manager->register(new TheGem_TemplateProductArchiveDescription());
}