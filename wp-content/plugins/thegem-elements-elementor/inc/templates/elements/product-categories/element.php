...
namespace TheGem_Elementor\Widgets\TemplateProductCategories;
...
/**
 * Elementor widget for Product Categories.
 */

#[\AllowDynamicProperties]
class TheGem_TemplateProductCategories extends Widget_Base {
	...
	public function get_name() {
		return 'thegem-template-product-categories';
	}
	...
	public function get_widget_wrapper() {
		return 'thegem-te-product-categories';
	}
	...
	public function get_customize_class($only_parent = false) {
		...
		return ' .'.$this->get_widget_wrapper().' .product-cats';
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'label_block' => false,
				'selectors' => [
					'{{WRAPPER}} .thegem-te-product-categories .product-cats__title' => 'color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'label_block' => false,
				'selectors' => [
					'{{WRAPPER}} .thegem-te-product-categories .product-cats__list' => 'color: {{VALUE}};',
					'{{WRAPPER}} .thegem-te-product-categories .product-cats__list a' => 'color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'label_block' => false,
				'selectors' => [
					'{{WRAPPER}} .thegem-te-product-categories .product-cats__list a:hover' => 'color: {{VALUE}};',
				],
			]
		);
	...
	public function render() {
		...
		$titleOutput = !empty($params['title']) ? '<span class="product-cats__title">'.esc_html($params['title']).': </span>' : null;
        ...
        <div <?php if (!empty($params['element_id'])): ?>id="<?=esc_attr($params['element_id']); ?>"<?php endif;?>
	        ...
	        <?= wc_get_product_category_list( $product->get_id(), ', ', '<div class="product-cats '.$text_styled_class.'">'.$titleOutput.'<span class="product-cats__list">', '</span></div>' ); ?>
        </div>
...
if(defined('WC_PLUGIN_FILE')) {
	\Elementor\Plugin::instance()->widgets_manager->register(new TheGem_TemplateProductCategories());
}