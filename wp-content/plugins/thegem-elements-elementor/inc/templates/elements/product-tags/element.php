...
namespace TheGem_Elementor\Widgets\TemplateProductTags;
...
/**
 * Elementor widget for Product Tags.
 */

#[\AllowDynamicProperties]
class TheGem_TemplateProductTags extends Widget_Base {
	...
	public function get_name() {
		return 'thegem-template-product-tags';
	}
	...
	public function show_in_panel() {
		...
		return get_post_type($post_id) === 'thegem_templates' && thegem_get_template_type($post_id) === 'single-product';
	}
	...
	public function get_customize_class($only_parent = false) {
		...
		return ' .'.$this->get_widget_wrapper().' .product-tags';
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}}'.$this->get_customize_class().' .product-tags__title' => 'letter-spacing: {{SIZE}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}}'.$this->get_customize_class().' .product-tags__title' => '{{VALUE}};',
				],
			]
		);
		...
		$this->add_group_control(
			...
			[
				'name' => 'typography',
				'selector' => '{{WRAPPER}}'.$this->get_customize_class().' .product-tags__title',
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'label_block' => false,
				'selectors' => [
					'{{WRAPPER}} '.$this->get_customize_class().' .product-tags__title' => 'color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'label_block' => false,
				'selectors' => [
					'{{WRAPPER}} '.$this->get_customize_class().' .product-tags__list a' => 'color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			'link_color_hover',
			[
				'label' => __('Link Color Hover', 'thegem'),
				'type' => Controls_Manager::COLOR,
				'label_block' => false,
				'selectors' => [
					'{{WRAPPER}} '.$this->get_customize_class().' .product-tags__list a:hover' => 'color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'label_block' => false,
				'selectors' => [
					'{{WRAPPER}} '.$this->get_customize_class().' .product-tags__list a' => 'background-color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				'label' => __('Link Background Color Hover', 'thegem'),
				'type' => Controls_Manager::COLOR,
				'label_block' => false,
				'selectors' => [
					'{{WRAPPER}} '.$this->get_customize_class().' .product-tags__list a:hover' => 'background-color: {{VALUE}};',
				],
			]
		);
	...
	public function render() {
		...
		$title_output = !empty($params['title']) ? '<span class="product-tags__title '.$text_styled_class.'">'.esc_html($params['title']).': </span>' : null;
		$tags_output = wc_get_product_tag_list( $product->get_id(), ' ', '<div class="product-tags">'.$title_output.'<span class="product-tags__list">', '</span></div>' );
...
if(defined('WC_PLUGIN_FILE')) {
	\Elementor\Plugin::instance()->widgets_manager->register(new TheGem_TemplateProductTags());
}