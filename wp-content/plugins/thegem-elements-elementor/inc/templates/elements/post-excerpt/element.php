...
namespace TheGem_Elementor\Widgets\TemplatePostExcerpt;
...
/**
 * Elementor widget for Post Description.
 */

#[\AllowDynamicProperties]
class TheGem_TemplatePostExcerpt extends Widget_Base {
	...
	public function get_name() {
		return 'thegem-template-post-excerpt';
	}
	...
	public function get_title() {
		...
		if(get_post_type($post_id) === 'thegem_templates' && thegem_get_template_type($post_id) === 'loop-item') {
			return __('Post Excerpt/Product Short Description', 'thegem');
		}
		return __('Post Excerpt', 'thegem');
	}
	...
	public function get_customize_class($only_parent = false) {
		...
		return ' .'.$this->get_widget_wrapper().' .post-excerpt';
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			'truncate_excerpt',
			[
				'label' => __('Truncate Excerpt (Lines)', 'thegem'),
				'type' => Controls_Manager::NUMBER,
				'min' => 1,
				'max' => 10,
				'step' => 1,
				'selectors' => [
					'{{WRAPPER}}'.$this->get_customize_class() => 'max-height: initial; white-space: initial; display: -webkit-box; -webkit-line-clamp: {{VALUE}}; line-clamp: {{VALUE}}; -webkit-box-orient: vertical; overflow: hidden; text-overflow: ellipsis;',
				],
			]
		);
	...
	public function render() {
		...
		$excerpt = '';
		$page_data = thegem_get_output_page_settings($single_post->ID);
		if ( !empty( $page_data['title_excerpt'] ) ) {
			$excerpt = $page_data['title_excerpt'];
		} elseif(has_excerpt($single_post->ID)) {
			$excerpt = preg_replace('%&#x[a-fA-F0-9]+;%', '', apply_filters('the_excerpt', get_the_excerpt($single_post->ID)));
		}

		if (empty($single_post) || empty($excerpt)) {
			ob_end_clean();
		...
		<div class="<?= esc_attr($params['element_class']); ?>">
			<<?= $params['text_tag'] ?> class="post-excerpt <?= $text_styled_class ?>">
				<?= $excerpt; ?>
			</<?= $params['text_tag'] ?>>
		</div>
...
\Elementor\Plugin::instance()->widgets_manager->register(new TheGem_TemplatePostExcerpt());
