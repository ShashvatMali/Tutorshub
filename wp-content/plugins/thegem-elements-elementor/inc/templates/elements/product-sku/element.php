...
namespace TheGem_Elementor\Widgets\TemplateProductSKU;
...
/**
 * Elementor widget for Product SKU.
 */

#[\AllowDynamicProperties]
class TheGem_TemplateProductSKU extends Widget_Base {
	...
	public function get_name() {
		return 'thegem-template-product-sku';
	}
	...
	public function get_customize_class($only_parent = false) {
		...
		return ' .'.$this->get_widget_wrapper().' .product-sku';
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			'title',
			[
				'label' => __('SKU Title', 'thegem'),
				'type' => Controls_Manager::TEXT,
				'default' => __('SKU', 'thegem'),
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'label_block' => false,
				'selectors' => [
					'{{WRAPPER}} .thegem-te-product-sku .product-sku__title' => 'color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'label_block' => false,
				'selectors' => [
					'{{WRAPPER}} .thegem-te-product-sku .product-sku__list' => 'color: {{VALUE}};',
				],
			]
		);
	...
	public function render() {
		...
		if (empty($product) || !wc_product_sku_enabled() || !$product->get_sku()) {
			ob_end_clean();
		...
		$title_output = !empty($params['title']) ? '<span class="product-sku__title">'.esc_html($params['title']).': </span>' : null;
		...
		<div <?php if (!empty($params['element_id'])): ?>id="<?=esc_attr($params['element_id']); ?>"<?php endif;?>
			...
			<div class="product-sku <?= $text_styled_class ?>" data-sku="<?= ($sku = $product->get_sku()) ? $sku : '' ?>">
				<?= $title_output ?> <span class="product-sku__list" itemprop="sku"><?= ( $sku = $product->get_sku() ) ? $sku : esc_html__( 'N/A', 'woocommerce' ); ?></span>
			</div>
		</div>
...
if(defined('WC_PLUGIN_FILE')) {
	\Elementor\Plugin::instance()->widgets_manager->register(new TheGem_TemplateProductSKU());
}