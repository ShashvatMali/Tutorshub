...
namespace TheGem_Elementor\Widgets\TemplateLoopPostContent;
...
class TheGem_TemplateLoopPostContent extends Widget_Base {
	...
	public function get_name() {
		return 'thegem-template-loop-post-content';
	}
	...
	public function get_widget_wrapper() {
		return 'thegem-te-loop-post-content';
	}
	...
	public function get_customize_class($only_parent = false) {
		...
		return ' .'.$this->get_widget_wrapper().' .post-content';
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			'truncate_content',
			[
				'label' => __('Truncate Content (Words)', 'thegem'),
	...
	public function render() {
		...
		$params['truncate_content'] = !empty($params['truncate_content']) ? intval($params['truncate_content']) : 20;
		if($params['truncate_content'] < 1 || $params['truncate_content'] > 100) {
			$params['truncate_content'] = 20;
		}

		ob_start();
		$single_post = thegem_templates_init_post();

		if (empty($single_post) || empty(get_the_content($single_post->ID))) {
			ob_end_clean();
		...
		<div class="<?= esc_attr($params['element_class']); ?>">
			<<?= $params['text_tag'] ?> class="post-content <?= $text_styled_class ?>">
				<?= wp_trim_words(apply_filters( 'the_content', get_the_content($single_post->ID)), $params['truncate_content']); ?>
			</<?= $params['text_tag'] ?>>
		</div>
...
\Elementor\Plugin::instance()->widgets_manager->register(new TheGem_TemplateLoopPostContent());
