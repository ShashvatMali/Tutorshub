...
class TheGemBlocks {
    ...
    public function import() {
        ...
        if (!$isIncludeMedia) {
            ...
            foreach($importPostIds as $iPId) {
                if(empty($this->importPosts[$iPId])) {
                    $importPostIdsUpd[] = $iPId;
                }
            }
        } else {
            $importPostIdsUpd = $importPostIds;
        }
        ...
        if (!empty($importPostIdsUpd)) {
            ...
            ob_start();
            $wpImporter->import(THEGEM_BLOCKS_DIR.'data/content.xml', $importPostIdsUpd, $importData['terms']);
            $result = ob_get_clean();
